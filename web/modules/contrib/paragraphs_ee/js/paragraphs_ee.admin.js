(function ($, Drupal, drupalSettings, once) {
  ...
  Drupal.paragraphs_features.add_in_between.initParagraphsWidget = function (context, field) {
    ...
    const buttonsAll = dialog.querySelectorAll('.paragraphs-button--add-more:not([data-paragraphs-ee-button-clone])');
    ...
    if (('dialogOffCanvas' in dialog.dataset) && (dialog.dataset.dialogOffCanvas === 'true')) {
      let wrappers = tableBody.querySelectorAll('.paragraphs-features__add-in-between__wrapper');
  ...
  Drupal.paragraphsAddModal.openDialog = function (element, title, options) {
    ...
    $(once('paragraphs-ee-dialog-submit', '.field-add-more-submit', $element.get(0))).on('mousedown', function (event) {
